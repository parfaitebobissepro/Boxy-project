...
export default function Shadow({ panelNumber, shadow }) {
    ...
    return (
        <li className='bg-gray-50 border-b border-gray-300'>
            <button className='px-6 py-4 flex justify-between items-center hover:bg-gray-100 w-full' onClick={() => setToggleShadow(!toggleShadow)}>
                <span>Shadow {panelNumber}</span>
                <img style={{ transform: `${toggleShadow ? "rotate(90deg)" : "rotate(0deg)"}` }} className='font-bold w-5' src={chevron} alt="" />
            </button>
            {toggleShadow &&
                <>
                    <div className='flex items-end px-6 pt-0'>
