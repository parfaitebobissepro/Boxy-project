...
export default function ModalResult({ closeModal }) {
    ...
    return (
        <div onClick={closeModal} className='fixed z-10 inset-0 flex items-center justify-center
     bg-gray-600/75'>
            <div onClick={e => e.stopPropagation()} className='max-w-[400px] rounded p-7 bg-gray-50 mb-[10vh]'>
                <div className="flex items-end mb-5">
                    ...
                    <button onClick={handleCopy} className='ml-auto mr-2 text-sm text-white bg-blue-600 hover:bg-blue-700 py-1 px-3 rounded'>Copy</button>
                    <button onClick={closeModal} className='mr-2 text-sm bg-red-600 text-white hover:bg-red-700 py-1 px-3 rounded'>Close</button>
                </div>
                <p className='round bg-gray-100 p-5'>
